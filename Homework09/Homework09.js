...
// GUI
const gui = new GUI();
const cameraFolder = gui.addFolder('Camera UI');
...
const cameraParams = {
    ...
    switchCamera: () => {
        ...
        if (isPerspective) {
            ...
            cameraParams.currentCamera = 'Orthographic';
        } else {
            camera = new THREE.PerspectiveCamera(45, aspect, 0.1, 100000);
            cameraParams.currentCamera = 'Perspective';
        }
...
cameraFolder.add(cameraParams, 'switchCamera').name('Switch Camera');
cameraFolder.add(cameraParams, 'currentCamera').name('Current Camera').listen();
